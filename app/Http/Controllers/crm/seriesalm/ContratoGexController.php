...
{
    //! Garantía extendida, producto marcita pro_codigo = 'GAREXT'
    private const PRO_ID_GAREXT = 8008634;
    ...
    public function loadInitialData($almId)
    {
        try {
            ...
            $facturas = DB::select("SELECT p.alm_id,
                        ...
                        c.cfa_id as cfa_id_gex,
                        concat(t.cti_sigla,' - ', a.alm_codigo, ' - ', p.pve_numero, ' - ',  c.cfa_numero) as factura_gex,
                        coalesce(cg.num_meses, 12) as meses_gex,
                        cast(c.cfa_fecha + 366 * interval'1 day' as date) as fecha_desde,
                        cast((c.cfa_fecha + 366 * interval'1 day') + coalesce(cg.num_meses, 12) * interval'1 month' as date) as fecha_hasta,
                        (select pc.km_garantia from gex.producto_config pc where pc.pro_id = pr.pro_id limit 1) as km_garantia,
                        (2) as km_factor,
                        (select pc.tipo_servicio from gex.producto_config pc where pc.pro_id = pr.pro_id limit 1) as tipo_servicio,
                        d.id_producto_gex
                from cfactura c 
                                join crm.av_cfactura_cnotacre_lineal c4 on c4.cfa_id = c.cfa_id
                                join dfactura d on c.cfa_id = d.cfa_id AND c4.ccm_estado = 2 AND c.cfa_periodo >= 2022 and c4.alm_id = ?
                                    -- nunca ofrecer como contratable la línea que ES la garantía
                                    and coalesce(d.prod_gex, false) = false
                                    and d.pro_id <> ?
                                    -- vía A: cruce clásico con cgex · vía B: la factura trae el producto GAREXT
                                    and (
                                        exists (select 1 from cgex cg2
                                                where cg2.id_dfactura = d.dfac_id and cg2.pro_id_gex = d.id_producto_gex)
                                        or exists (select 1 from dfactura g
                                                    where g.cfa_id = d.cfa_id and g.pro_id = ?)
                                    )
                                --JOIN crm.contrato_gex cg1 ON cg1.cfa_id = c.cfa_id AND cg1.pro_id = d.pro_id AND cg1.deleted_at IS NULL
                                join puntoventa p on c.pve_id = p.pve_id
                                join almacen a on p.alm_id = a.alm_id
                                ...
                                join v_entidad ve on e.ent_id = ve.ent_id
                                join v_ubicacion vu on c1.ubi_id = vu.ubi3_id --c.cfa_id = 446464
                                left join cgex cg on cg.id_dfactura = d.dfac_id and cg.pro_id_gex = d.id_producto_gex
                where not exists (select 1 from crm.contrato_gex cgc where cgc.cfa_id = d.cfa_id and cgc.pro_id = d.pro_id)
                --and not exists (select 1 from gex.contrato_gex cg where cg.cfa_id = d.cfa_id and cg.pro_id = d.pro_id)
                group by p.alm_id, a.alm_nombre, e.ent_nombres, e.ent_apellidos, ve.ent_tipo_identificacion, ve.ent_identificacion, ve.dir_calle_principal, ve.dir_numeracion,
                        ve.dir_calle_secundaria, vu.ubi_nombre, vu.ubi2_nombre, e.ent_email, pr.pro_id, tp.tpr_nombre, pr.pro_nombre, c.cfa_id, t.cti_sigla, a.alm_codigo, p.pve_numero,
                        c.cfa_numero, c.cfa_fecha, m.mar_nombre, coalesce(cg.num_meses, 12), c.cfa_fecha, a.ubi_id, e.ent_telefono_principal, e.ent_id, d.id_producto_gex;",
                        [$almId, self::PRO_ID_GAREXT, self::PRO_ID_GAREXT]);
            $data = (object)[
                "facturas" => $facturas
            ];
